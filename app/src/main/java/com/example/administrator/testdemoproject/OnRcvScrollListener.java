...
/**
 * Created by MrDong on 2016/11/10.
 */
...
public abstract class OnRcvScrollListener extends RecyclerView.OnScrollListener {
    ...
    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
        ...
        if (visibleItemCount > 0) {
            if (mLastVisibleItemPosition >= totalItemCount - 1 && newState == RecyclerView.SCROLL_STATE_IDLE && layoutManager.getDecoratedBottom(layoutManager.getChildAt(visibleItemCount - 1)) == layoutManager.getHeight()) {
                onBottom();
            } else if (newState == RecyclerView.SCROLL_STATE_DRAGGING && layoutManager.getDecoratedBottom(layoutManager.getChildAt(visibleItemCount - 1)) >= layoutManager.getHeight()) {
                onScrollLoad();
            }
        }
    }
